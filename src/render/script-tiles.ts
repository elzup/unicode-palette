...
export function generateTileData(
  blocks?: UnicodeBlock[],
  bmpOnly = true
): TileData[] {
  const targetBlocks = blocks ?? (bmpOnly ? BMP_BLOCKS : UNICODE_BLOCKS)
...
export function generateScriptTilesSvg(options: TileOptions = {}): string {
  const { cellSize, cols, fontPath, blocks, bmpOnly } = {
    ...DEFAULT_TILE_OPTIONS,
    ...options,
  }
  const tileData = generateTileData(blocks, bmpOnly)
  ...
  tileData.forEach((tile, index) => {
    ...
    // Background
    tiles.push(
      `<rect x="${x}" y="${y}" width="${tileSize}" height="${tileSize}" fill="#f0f0f0" stroke="#ccc" stroke-width="0.5"/>`
    )
    ...
    tile.chars.forEach((charInfo, i) => {
      ...
      if (charInfo.printable) {
        tiles.push(
          `<text x="${cx}" y="${cy}" class="char">${escapeXml(charInfo.char)}</text>`
        )
      } else {
        // Show codepoint for non-printable
        const hex = charInfo.codepoint
          .toString(16)
          .toUpperCase()
          .padStart(4, '0')
    ...
    tiles.push(
      `<text x="${x + tileSize / 2}" y="${labelY}" class="label">${escapeXml(tile.block.displayName)}</text>`
    )
  })
